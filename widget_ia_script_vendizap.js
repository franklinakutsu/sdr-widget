(function () {
  ...
  // data-exibir ausente => exibe a UI normalmente (botão flutuante visível).
  // data-exibir="false"/"0" => UI oculta, controlada externamente via window.SdrChat.
  ...
  function sdrToggle() {
    ...
    } catch (e) { /* CustomEvent indisponível em ambientes muito antigos */ }
  }
  ...
  window.SdrChat = {
    ...
    setEmail: function (email) {
      if (typeof email === 'string' && email) SDR_USER_EMAIL = email;
    }
  };
  ...
  function sdrSubmitCheckout(formId) {
    ...
    fetch(SDR_URL_CHECKOUT + '/checkout', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ lead_id: sdrLeadId, conversation_id: sdrConvId, plano: plano, periodo: periodo })
    })
    .then(function (r) { return r.json(); })
    .then(function (data) {
